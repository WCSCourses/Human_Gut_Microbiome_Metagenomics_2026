...
Poor-quality or contaminated reads can lead to:
- false taxa detection
- inflated functional profiles
- contaminant-driven genome assemblies
- misleading biologically conclusions

Quality control is therefore not optional, but a prerequisite for reliable metagenomic analysis.
...
4.	Remove host contamination from metagenomic datasets.
5.	Interpret FASTQC and MultiQC reports.
6.	Understand how preprocessing affects downstream metagenomic analyses.
7.	Explain why reproducible workflows are important in large-scale microbiome studies..
___
...
- Illumina sequencing→ short reads with relatively low error rate
...
> **NB:** upstream laboratory decisions often influence the quality patterns observed during computational preprocessing.
...
The International Nucleotide Sequence Database Collaboration (INSDC) consists three synchronized global archives:
...
Create a file containing one run accession per line:txt:
 ```text
...
---
## 1. Adapter trimming and quality filtering
During library preparation, short adapter sequences are attached to DNA fragments to enable sequencing. Fragments may retain these adapters, and sequencing quality often declines toward the 3′ end of reads.

Common issues addressed during trimming include:
- adapter contamination
- low-quality bases
- very short reads
- sequencing artefacts

Removing these artefacts improves mapping accuracy and downstream analyses.
___
...
## 3. Host DNA Removal
In host-associated microbiome datasets, a proportion of reads may originate from host cells rather than microbes.

Examples include:
- epithelial cells
- mitochondrial DNA
- host genomic fragments

If not removed, host reads can reduce analytical efficiency and introduce privacy concerns in human datasets.

In metaWRAP, host filtering is performed using BMTagger. Here we demonstrate the same principle using Bowtie2, which aligns reads to a host reference genome and retains only unmapped reads.
